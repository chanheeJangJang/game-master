...
public class stacking extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        AlertDialog.Builder gamestart_dlg=new AlertDialog.Builder(stacking.this);
            ...
            public void onClick(View v) {
                ...
                if(pattyCount==order_pattyCount) {//주문 햄버거의 패티갯수와 내가 제작한 패티갯수가 같을 경우
                    if (recipe_Code.equals(my_recipe_Code))//주문 햄버거의 레시피 코드와 내가 제작한 햄버거의 레시피 코드가 같을 경우
                    {
                        ...
                        Toast tMsg = Toast.makeText(stacking.this,"성공!!",Toast.LENGTH_SHORT);
                        tMsg.show();
                    }
                    else
                    {

                        Toast tMsg = Toast.makeText(stacking.this,"실패!!",Toast.LENGTH_SHORT);
        ...
        bread_bottom_btn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                my_recipe[pattyCount].setImageResource(R.drawable.bread_bottom);
                my_recipe_Code+="A";
                pattyCount++;
                if(pattyCount==order_pattyCount) {//주문 햄버거의 패티갯수와 내가 제작한 패티갯수가 같을 경우
                    if (recipe_Code.equals(my_recipe_Code))//주문 햄버거의 레시피 코드와 내가 제작한 햄버거의 레시피 코드가 같을 경우
    ...
    public  void timer_Start()
    {
        ...
        final TimerTask tt=new TimerTask() {
            ...
            public void run() {
                ...
                stacking.this.runOnUiThread(new Runnable() {//타이머에서 ui를 수정해야 할경우 runOnUiThread를 사용해야함
                    @Override
                    public void run() {
                        if(timer_Count<0) {//타이머가 0 미만이 될 경우
                            timer.cancel();

                            AlertDialog.Builder gameover_dlg=new AlertDialog.Builder(stacking.this);
                            gameover_dlg.setTitle("게임 종료");
                            //gameover_dlg.setMessage("만든 갯수 : "+success_Count.toString());
                            if(success_Count >=10){
                                score =3;
                                log2.updateUsers5(0,0,0,3,0);
                                gameover_dlg.setMessage("Perfect!");
                            }
                            else if (success_Count>=5){
                                score =2;
                                log2.updateUsers5(0,0,0,2,0);
                                gameover_dlg.setMessage("Good!");
                            }
                            else{
                                score =1;
                                log2.updateUsers5(0,0,0,1,0);
                            ...
                            gameover_dlg.setPositiveButton("다음 단계로", new DialogInterface.OnClickListener() {
                                ...
                                public void onClick(DialogInterface dialog, int which) {
                                    timer_Start();
                                    createRecipe();
                                    Intent intent = new Intent(stacking.this, cream.class);
    ...
    public void createRecipe()
    {
        ...
        //아래빵
        recipe[0].setImageResource(R.drawable.bread_bottom);
        recipe_Code+="A";
